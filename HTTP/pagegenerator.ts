// 8 x 8
function getRow(num: number) { return Math.floor(num / 8) }
function getX(num: number) { return num % 8 }


function getFieldDesc(num: number, onlyside?: boolean) {
    let x = getX(num);
    let y = 8 - getRow(num);
...
function createField(chessRow: ChessRow, white: boolean,j: number) {
...
let stylesheet = document.styleSheets[0];
    function getCSSRule(name: string): any {
        for (let i = 0; i < stylesheet.cssRules.length; i++) {
            let r: any = stylesheet.cssRules[i];
            if (r.selectorText == name) return r;
        }
    }
function setBoardColor(white: string, black: string, lastMove?: string) {
...
function getBoardColor() {
    ...
    return {
        ...
        setWhite: (value: string)=>root.setProperty("--whiteFieldColor", value),
        setBlack: (value: string)=>root.setProperty("--blackFieldColor", value),
        setLastMove: (value: string)=>lastMove.setProperty("--c",value),
        setAvaible: (value: string)=>avaible.setProperty("--c", value)
        
    }
}


function setCSSRule(selector: string, property: string, value: string) {
