...
//SOUND SETTINGS
{
    ...
    for (let key in SOUNDS) {
        //@ts-ignore
        let sound: HTMLAudioElement = SOUNDS[key];
        let play = sound.play;
        //@ts-ignore
        sound.play = () => {
            sound.pause();
...
interface Figure {
    type: FIGURE, color: COLOR, field: ChessField, alreadyMoved: boolean;
}

interface Move {
    start: number, end: number, figure: VirtualFigure, worth: number, target?: VirtualFigure, flag?: FLAG_DATA_SET;
}
...
let lastMovedElements: HTMLElement[] = [];
function highlightMoveElement(...ids: number[]) {
    lastMovedElements.forEach(e => {
        e.remove();
    });

    lastMovedElements = [];
    let f1: HTMLElement | null = null;
    let f2: HTMLElement | null = null;
    ids.forEach(id => {
        let fieldElement = chessBoard.fields[id].getElement();
        if (!f1) f1 = fieldElement;
        else if (!f2) f2 = fieldElement;
...
function doMove(move: Move) {
...

    {
        ...
        let div: HTMLElement = chessBoard.addToMoveContainer(move, startFig, f);
        ...
        onhover(div, {
            start: () => {
                ...
                function unhighlighHover(fig: ChessField) { fig.getElement().style.border = "" }
...
function getOtherColor(color: COLOR) {
    return color == COLOR.black ? COLOR.white : COLOR.black
}



function ran(to: number) {
    return Math.floor(Math.random() * to);
}


function setFocusOnDivWithId(element: HTMLElement) {
...
function onhover(trigger: HTMLElement, data: { view?: HTMLElement, start: Function, end: Function, click?: Function }) {
    let view = data.view;
    if (view) {
    ...
    //@ts-ignore
    if (data.click) trigger.addEventListener("click", e => data.click())
}
