class ChessBoard {
    ...
    private rows: ChessRow[] = [];
    private onTurn = COLOR.white;
    private rotation = 0;
    thisInstanceColor = COLOR.white;
    turns = 1;
    fields: ChessField[] = [];
    teams = { white: new Team(COLOR.white), black: new Team(COLOR.black) }
    ...
    rotate(r?: number) {
        if (!r) r = this.rotation += 180;
        function rot(e: HTMLElement) {
    ...
    setCSSBoardSize(size: number) {
        setCSSRule("#board", "--cw", `${size}px`);
    }

    addToMoveContainer(move: Move, startFig: Figure, f: Figure | null) {
    ...
    loadFen(fen: string) {
        ...
        function figFromFenChar(value: string, field: ChessField): Figure {
            let isWhite = value == value.toUpperCase();

            //@ts-ignore
            let type: FIGURE = fenChars[value.toLocaleLowerCase()];
        ...
        if (onturn == "b") {
            if (this.getCurrentSide() == COLOR.white) this.toggleTurn();
        } else if (onturn == "w") {
            if (this.getCurrentSide() == COLOR.black) this.toggleTurn();
        } else {
