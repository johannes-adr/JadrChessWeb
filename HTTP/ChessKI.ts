...
class VirtualFigure {
    ...
    chessField;
    alreadyMoved;
    constructor(type: FIGURE, color: COLOR, alreadyMoved: boolean, chessField: ChessField, startPos: number, virtualBoard: VirtualBoard) {
        this.color = color;
        this.type = type;
    ...
    clone(virtualBoard?: VirtualBoard): VirtualFigure {
    ...
    doMove(move: Move) {
        ...
        let dp: null | (VirtualFigure | undefined)[];
        let i: number | null;
        let changed = -1;
        if (endPiece) {
        ...
        return {
            move: move, undo: () => {
                ...
                if(changed != -1)vb.directPieces[endPiece!.color][changed] = endPiece;
                t.alreadyMoved = movedBefore;
                if (endPiece) dp![i!] = endPiece;
            }
        };
    }

    getAvaibleMoves(doKingCheck: boolean) {
        ...
        function addMove(end: number, worth?: number, flag?: FLAG_DATA_SET) {
            if (end < 0 || end >= fields.length) return
            let vb2 = new VirtualBoard(t.virtualBoard);

            if (!worth) worth = 0;
            let endRow = getRow(end);
            if (endRow > fy) worth += (endRow - fy) * 5;
            let x = getX(end);
            let centerVal = x < 5 ? 7 - x : x;
            worth += 8 - centerVal / t.virtualBoard.turns * 5;

            let oField = fields[end];
            if (oField) worth = oField.worth;
            let move: Move = { start: t.pos, end: end, figure: t, worth: worth }
            if (flag) move.flag = flag;
            if (oField) move.target = oField;
            ...
            if (doKingCheck) {
                ...
                if(flag && flag.flag ==FLAG.CASTLE){
                    if(vb2.isFigureTypeUnderAttack(FIGURE.king,t.color))return;
                }
            ...
            if (oField?.color != t.color) moves.push(move);
        }

        function xInBounds(x: number) { return !(x > 7 || x < 0) }


        function generateRookMoves() {
            //DOWN
            for (let ly = fy + 1; ly < 8; ly++) {
                let li = ly * 8 + fx; addMove(li);
                if (fields[li] != null) break;
            }
            //UP
        ...
        function checkPawnPromote(isBlack: boolean, index: number) {
            if (isBlack){
        ...
        if (this.type == FIGURE.king) {
            for (let y = -1; y < 2; y++) {
                ...
                for (let x = -1; x < 2; x++) {
                    ...
                    if (r != getRow(localI) || localI == i) continue;
                    addMove(localI);
                }
            }
            ...
            if (fields[front] == null) {
                ...
                if (b) {
                    if (fy == 1 && fields[front + 8] == null) addMove(front + 8, worth);
                } else {
                    if (fy == 6 && fields[front - 8] == null) addMove(front - 8, worth);
                }
            }
            ...
            generateBishopMoves(); generateRookMoves();
...
class VirtualBoard {
    pieces: (VirtualFigure | undefined)[] = [];
    directPieces: { white: (VirtualFigure | undefined)[], black: (VirtualFigure | undefined)[] } = { white: [], black: [] };
    turns = 0;
    constructor(init: Boolean | ChessBoard | VirtualBoard) {
        if (init instanceof ChessBoard) {
            ...
            for (let i = 0; i < init.pieces.length; i++) {
                ...
                if (piece) this.directPieces[piece.color].push(piece);
            }
            this.turns = init.turns;
        }
    }

    getPiece(figure: VirtualFigure) {
        return this.pieces[figure.pos];
    }

    isFigureUnderAttack(fig: VirtualFigure) {
        ...
        for (let i = 0; i < enemyPieces.length; i++) {
            ...
            for (let j = 0; j < moves.length; j++) {
                let move = moves[j];
                let t = move.target;
                if (t == fig) return move;
            }
        }
        return null;
    }

    isFigureTypeUnderAttack(fig: FIGURE, color: COLOR) {
    ...
    getBestMove(maxLevel: number, color: COLOR) {
        var calc = 0;
        let t = this;
        function getBestMoveRecursive(vb: VirtualBoard, color: COLOR, maxLevel: number, currentLevel: number) {
                ...
                for (let j = 0; j < moves.length; j++) {
                    ...
                    if (bestMove == null) bestMove = move;
                        ...
                        if (!counterMove) {
                            ...
                            if (move.worth >= counterMove.worth) bestMove = move;
                        }
                    }
                    else if (bestMove.worth == move.worth && Math.random() > 0.7) bestMove = move;
                }
            }
